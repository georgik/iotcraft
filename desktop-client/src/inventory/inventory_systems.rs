...
pub fn place_block_system(
    ...
    for event in events.read() {
        // Determine block type: either from event (for MCP commands) or from selected inventory item
        let (block_type, should_consume_inventory) =
            if let Some(event_block_type) = event.block_type {
                // MCP command specified the block type - use it directly
                (event_block_type, false)
            } else if let Some(selected_item) = inventory.get_selected_item() {
                // Regular UI placement - use selected inventory item
                let ItemType::Block(block_type) = selected_item.item_type;
                if selected_item.count > 0 {
                    (block_type, true)
                } else {
                    continue; // No items to place
                }
            } else {
                continue; // No selected item and no block type specified
            };
        ...
        // Remove item from inventory only if this was a regular UI placement
...
pub fn place_block_multiplayer_sync_system(
    ...
    for event in place_block_events.read() {
        ...
        if is_multiplayer_with_world {
            ...
            info!(
                "✅ In multiplayer mode (world_id: {}), determining block type for MQTT sync",
                world_id
            );

            // Get the block type either from the event (MCP commands) or from selected inventory item (UI)
            let block_type = if let Some(event_block_type) = event.block_type {
                // MCP command specified the block type - use it directly
                event_block_type
            } else if let Some(selected_item) = inventory.get_selected_item() {
                // Regular UI placement - use selected inventory item
                ...
                warn!(
                    "❌ No block type specified in event and no selected item in inventory for position {:?}",
                    event.position
                );
            ...
            info!(
                "🧱 Using block type: {:?}, generating MQTT event for player {} ({})",
