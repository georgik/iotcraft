...
fn discover_worlds(mut discovered_worlds: ResMut<DiscoveredWorlds>) {
    ...
    worlds.sort_by(|a, b| {
        b.metadata.last_played.cmp(&a.metadata.last_played)
    });
...
fn load_world_info(world_path: &Path) -> Option<WorldInfo> {
    ...
    match fs::read_to_string(&metadata_path) {
        Ok(content) => {
            match serde_json::from_str::<WorldMetadata>(&content) {
...
fn handle_load_world_events(
    ...
    for event in load_events.read() {
        ...
        if let Some(world_info) = discovered_worlds.worlds.iter().find(|w| w.name == event.world_name) {
            ...
            if world_data_path.exists() {
                ...
                match fs::read_to_string(&world_data_path) {
                    Ok(content) => {
                        match serde_json::from_str::<WorldSaveData>(&content) {
                            Ok(save_data) => {
                                info!("Loaded world save data with {} blocks", save_data.blocks.len());
                                for (i, block_data) in save_data.blocks.iter().take(5).enumerate() {
                                    info!("  Block {}: {:?} at ({}, {}, {})", i, block_data.block_type, block_data.x, block_data.y, block_data.z);
                                }
                                ...
                                info!("Cleared {} existing block entities from scene", cleared_entities);
                            ...
                            Err(e) => error!("Failed to parse world data for {}: {}", event.world_name, e),
                        }
                    }
                ...
                create_empty_world(&event.world_name, &world_info.metadata.description, &mut voxel_world, &mut commands, &world_info.path);
            }
        } else {
...
fn handle_save_world_events(
    ...
    for event in save_events.read() {
        ...
        if let Some(current_world) = &current_world {
            ...
            let (player_position, player_rotation) = if let Ok(transform) = camera_query.get_single() {
            ...
            info!("VoxelWorld contains {} blocks before saving", voxel_world.blocks.len());
            ...
            match serde_json::to_string_pretty(&save_data) {
                ...
                Err(e) => error!("Failed to serialize world data for {}: {}", event.world_name, e),
            }
        } else {
...
fn handle_create_world_events(
    ...
    for event in create_events.read() {
        ...
        if let Err(e) = fs::create_dir_all(&world_path) {
            error!("Failed to create world directory for {}: {}", event.world_name, e);
            continue;
        }
        ...
        match serde_json::to_string_pretty(&metadata) {
            ...
            Err(e) => {
                error!("Failed to serialize metadata for {}: {}", event.world_name, e);
                continue;
            }
        }
        
        // Create empty world
        create_empty_world(&event.world_name, &event.description, &mut voxel_world, &mut commands, &world_path);
