...
// Texture size is now dynamic (loaded from SD card or embedded)
#define TEXTURE_SIZE texture_get_size()
    ...
    for (int32_t x = start_col; x < end_col; x++) {
        ...
        for (int32_t f = 0; f < face_count; f++) {
            ...
            if (side == 2) {
                ...
                // Get texture
                const uint16_t* texture = texture_get(block_type);
                if (!texture) continue;
...
static void draw_voxel_3d(renderer_t* renderer, int32_t x, int32_t y, int32_t z, block_type_t block) {
    ...
    const uint16_t* texture = texture_get(block);
    if (!texture) return;
